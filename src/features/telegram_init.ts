import {init, miniApp, mockTelegramEnv, parseInitData, mainButton, shareURL} from "@telegram-apps/sdk";

const initializeTelegramSDK = async () => {
  try {
    // Попытка инициализировать настоящее окружение Telegram
    console.log("Инициализация окружения Telegram");
    const [miniApp]: any = init();
    await miniApp.ready();
  } catch (error) {
    // В случае ошибки инициализируем фейковое окружение
    console.error('Ошибка при инициализации Telegram:', error);
    alert(`Ошибка при инициализации Telegram: ${error.toString()}`);
  ...
  miniApp.mount()
  miniApp.setHeaderColor('#883a3a');


  mainButton.mount()
  mainButton.setParams({
    backgroundColor: '#e715a9',
    text: 'Поделиться игрой',
    isVisible: true,
    isEnabled: true,
  });
  ...
  mainButton.onClick( () => {
    try {
      shareURL(`хей заходи играть в наш дуолинго чел`);
